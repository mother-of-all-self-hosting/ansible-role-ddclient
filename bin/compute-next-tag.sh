...
#
# Tags look like `v<ddclient version>-<release>`:
#
# - if defaults/main.yml points at a ddclient version that has never been
#   released, the release counter restarts at 0 (`v4.1.0-0`)
# - otherwise the counter is incremented (`v4.1.0-1`), but only if something
...
# `ddclient_version` is the leaf value that Renovate bumps. The other version
# variables in defaults/main.yml (`ddclient_container_image_tag` and
# `ddclient_container_image_self_build_repo_version`) are Jinja expressions
# derived from it, so matching on the leaf is what keeps this correct.
version="$(sed -nE 's|^ddclient_version:[[:space:]]*"?([^"[:space:]]+)"?.*$|\1|p' "$defaults_path" | head -n1)"

if [ -z "$version" ]; then
	echo >&2 "Could not determine the ddclient version from $defaults_path"
	exit 1
fi

# The version values do not carry a leading `v` (e.g. `4.0.0`), but the tags
# do (`v4.0.0-0`). Stripping any `v` before prepending one keeps this correct
